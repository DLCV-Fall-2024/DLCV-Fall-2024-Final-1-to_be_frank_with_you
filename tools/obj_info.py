...
def arg_parser():
    parser = argparse.ArgumentParser(description="Generate object information")
...
def examine_path(dataset_path: str):
    ...
    if Path(dataset_path).name not in target_dir:
        subdirs = [x for x in Path(dataset_path).iterdir() if x.is_dir()]
        for subdir in subdirs:
            if subdir.name in target_dir:
                get_image_path(subdir)
    else:
        get_image_path(dataset_path)


def get_image_path(dataset_path="data/val"):
    batch_size = 16
    prefetch_factor = 8
    num_workers = 10
...
def main():
    args = arg_parser()
    examine_path(args.dataset_path)
